...
class OrganizationForEvent extends Component {

  constructor(props, context) {
    super(props)

    const {modelType, forObjectId, eventId} = props.params;
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const {modelType, forObjectId, eventId} = nextProps.params;
    ...
    this.setState({
      // Detailed object
      forObject: getModelByObjectId(nextProps, eventId, this.state.forObject),
  ...
  componentDidMount() {
    const {pageForm, eventId} = this.state;
    ...
    if (pageForm === MODEL_FORM_TYPE_EDIT) {
      this.props.dispatch(loadOrderedRecipePage(eventId))
    }
  }
...
export default withRouter(connect(select)(OrganizationForEvent));
